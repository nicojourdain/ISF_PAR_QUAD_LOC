MODULE isf_oce
   ...
   PUBLIC   isf_alloc, isf_alloc_par, isf_alloc_par_quad, isf_alloc_cav, isf_alloc_cpl
   !
   !-------------------------------------------------------
...
CONTAINS
   SUBROUTINE isf_alloc_par_quad(cloc)
      ...
      IF (TRIM(cloc) == 'glo') THEN
         !
         ALLOCATE(id_basin_isfpar(A2D(0)), STAT=ialloc)
         ierr = ierr + ialloc
      ...
      ELSE IF (TRIM(cloc) == 'loc') THEN 
         ...
         ierr = ierr + ialloc
         ALLOCATE(mskisf_exchg(A2D(0),nbasins_loc), STAT=ialloc)
         ierr = ierr + ialloc
   ...
   SUBROUTINE isf_alloc_par()
      ...
      ALLOCATE( misfkt_par  (A2D(1)) , misfkb_par(A2D(1)) , rfrac_tbl_par (A2D(1)) ,                               &
         &     rhisf_tbl_par(A2D(1)) , mskisf_par(A2D(0)) , rhisf0_tbl_par(A2D(1)) , risfLeff(A2D(0)), STAT=ialloc )
      !
      CALL mpp_sum ( 'isf', ialloc )
      IF( ialloc /= 0 )   CALL ctl_stop( 'STOP', 'isf: failed to allocate arrays (par).' )
      !
   ...
   SUBROUTINE isf_alloc_cav()
      ...
      !
      CALL mpp_sum ( 'isf', ialloc )
      IF( ialloc /= 0 )   CALL ctl_stop( 'STOP', 'isf: failed to allocate arrays (cav).' )
      !
