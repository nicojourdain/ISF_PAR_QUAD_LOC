MODULE isf_oce
   ...
   !-------------------------------------------------------
   !
   REAL(wp), PARAMETER, PUBLIC :: rLfusisf = 0.334e6_wp    !: latent heat of fusion of ice shelf     [J/kg]  ! nj: already defined as rLfus in phycst.F90
...
CONTAINS
   ...
   SUBROUTINE isf_alloc_par()
      ...
      !
      CALL mpp_sum ( 'isf', ierr )
      IF( ierr /= 0 )   CALL ctl_stop( 'STOP', 'isf: failed to allocate arrays (par).' )
      !
   ...
   SUBROUTINE isf_alloc_cav()
      ...
      !
      CALL mpp_sum ( 'isf', ierr )
      IF( ierr /= 0 )   CALL ctl_stop( 'STOP', 'isf: failed to allocate arrays (cav).' )
      !
